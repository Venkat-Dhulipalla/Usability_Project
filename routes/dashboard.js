                ...
                if (error) {
                    ...
                    if (results.length > 0) {
                        if (results[0].password === password) {
                            ...
                            var sql = `Select name from volunteer_profile where login_id=?`;
                            if (type == 1) {
                                sql = `Select name from admin_profile where login_id=?`;
                            }
...
router.post('/basic_complaint_detail',function(req, res, next) {
    ...
    req.getConnection(function (err, connection) {
        if (err) {
            ...
            connection.query("Select status, id from complaint where email = ?", [email], function (error, results) {
                if (error) {
                    console.log("error ocurred", error);
                } else {
                    if (results.length > 0) {
